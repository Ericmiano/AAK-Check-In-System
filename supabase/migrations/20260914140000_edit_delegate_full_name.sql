-- Staff should also be able to correct a delegate's name (typos, wrong
-- spelling variant from an import merge, etc.), not just fill in missing
-- contact details. full_name stays required — a blank value here means
-- "leave it as-is" like the other fields, not "clear the name".
drop function if exists public.update_delegate_details(uuid, text, text, text, boolean);
...
begin
  ...
  if v_name is not null and (length(v_name) < 2 or length(v_name) > 120) then
  ...
  perform public.log_audit('delegate.details_updated', 'delegate', d.id, jsonb_build_object(
    'full_name_set', v_name is not null,
...
revoke all on function public.update_delegate_details(uuid, text, text, text, text, boolean) from public, anon;
grant execute on function public.update_delegate_details(uuid, text, text, text, text, boolean) to authenticated;
